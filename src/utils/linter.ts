import type { LintRule, LintResult, StrictnessLevel, StrictnessOption, IssueSeverity } from '../types/lint';
...
const STRICTNESS_LEVELS: Record<StrictnessLevel, StrictnessLevel[]> = {
  relaxed: ["relaxed"],
...
export function lintMarkdown(markdown: string, strictness: StrictnessLevel = "standard"): LintResult {
  ...
  const allIssues: Array<{ line: number; column: number; message: string; ruleId: string; ruleName: string; ruleDescription: string; severity: IssueSeverity }> = [];
...
export function getAllRules(): Array<{ id: string; name: string; description: string; level: StrictnessLevel; severity: IssueSeverity }> {
...
export function getStrictnessLabel(level: StrictnessLevel): string {
